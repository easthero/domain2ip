...
class scanner(threading.Thread):
    ...
    def get_ipinfo(self, domain):
            try:
                ...
                if not ipaddr:
                    print("None of DNS query names exist: %s, add www to continue " %(domain))
                    domain = ('www.'+domain)
                    ipaddr = nslookup(domain)
                    if not ipaddr:
                       pass
                ...
                cnames = getcname(domain)
       #          print(domain + "cname is" + cnames)
                msg = ""
                ...
                if msg:
                    msg = msg[:-1]+"(open)"
                res = {'Hostname':domain, 'CNAME': cnames, 'IPAddr':ipaddr,'ISP':ipinfo['isp'],'Country':ipinfo['country'],'asn_cidr':ipinfo['asn_cidr'],'asn_description':ipinfo['asn_description'],'PortScan':msg,'StatusCode':status_code,'httpserver':httpserver,'Title':title}
...
def nslookup(domain):
   ...
   #     res = dns.resolver.Resolver.resolve(domain, "A")
...
def getcname(domain):
    try:
       ...
       # res = dns.resolver.Resolver.resolve(domain, "CNAME")
        for cnameval in res:
...
def save2csv(data,filename):
    with open(filename, 'w', newline='',encoding='utf-8-sig') as csvfile:
        fieldnames = ['Hostname', 'CNAME', 'IPAddr','Country','ISP','asn_cidr','asn_description','PortScan','StatusCode','httpserver','Title']
